// -----------------------------------------------------------------------
//  <copyright file="KafkaIntegrationTests.cs" company="Akka.NET Project">
...
public abstract class KafkaIntegrationTests : Akka.TestKit.Xunit.TestKit
{
    ...
    {
        get { return BuildProducerSettings<Null, string>(); }
    }

    protected ProducerSettings<TKey, TValue> BuildProducerSettings<TKey, TValue>() => ProducerSettings<TKey, TValue>
        .Create(Sys, null, null).WithBootstrapServers(Fixture.KafkaServer);
    ...
    protected ConsumerSettings<Null, TValue> CreateConsumerSettings<TValue>(string group) =>
        ConsumerSettings<Null, TValue>.Create(Sys, null, null)
            .WithBootstrapServers(Fixture.KafkaServer)
            .WithStopTimeout(TimeSpan.FromSeconds(1))
            .WithProperty("auto.offset.reset", "earliest")
            .WithGroupId(group);
    ...
    protected Task GivenInitializedTopicAsync(TopicPartition topicPartition,
        ...
        GivenInitializedTopicAsync(topicPartition.Topic, partitions);
