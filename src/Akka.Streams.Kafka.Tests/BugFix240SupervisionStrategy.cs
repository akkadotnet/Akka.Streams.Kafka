// -----------------------------------------------------------------------
//  <copyright file="BugFix240SupervisionStrategy.cs" company="Akka.NET Project">
...
public class BugFix240SupervisionStrategy : KafkaIntegrationTests
{
    ...
    public async Task Committable_consumer_with_failed_downstream_stage_result_should_be_gapless()
    {
        ...
        var count = 0;
        var (control, probe) = KafkaConsumer.CommittableSource(consumerSettings,
        ...
        // restart dead stream
        var (control2, probe2) = KafkaConsumer.CommittableSource(consumerSettings,
                Subscriptions.AssignmentWithOffset(new TopicPartitionOffset(topicPartition, Offset.Stored)))
            .AlsoToMaterialized(CreateSink(), Keep.Left)
            .Select(c => c.Record.Message.Value)
            .ToMaterialized(this.SinkProbe<string>(), Keep.Both)
            .Run(Materializer);
    ...
    public async Task SupervisionStrategy_Decider_on_complex_stream_should_work()
    {
        ...
        // act
        var drainingControl = KafkaConsumer
            .CommittableSource(consumerSettings, Subscriptions.Assignment(topicPartition))
    ...
    public async Task SupervisionStrategy_Decider_on_PlainSource_should_work()
    {
        ...
        var sourceTask = ProduceStrings(new TopicPartition(topic1, 0), Enumerable.Range(1, elementsCount),
            ProducerSettings);
    ...
    private class StringDeserializer : IDeserializer<string>
    {
        public string Deserialize(ReadOnlySpan<byte> data, bool isNull, SerializationContext context) =>
            BitConverter.ToInt32(data).ToString();
    }
