// -----------------------------------------------------------------------
//  <copyright file="PlainPartitionedSourceIntegrationTests.cs" company="Akka.NET Project">
...
public class PlainPartitionedSourceIntegrationTests : KafkaIntegrationTests
{
    ...
    public async Task PlainPartitionedSource_should_be_signalled_about_serialization_errors()
    {
        ...
        var subsources = partitionedProbe.Within(TimeSpan.FromSeconds(10),
            () => partitionedProbe.ExpectNextN(3).Select(t => t.Item2).ToList());
        var substream = subsources.Aggregate((s1, s2) => s1.Merge(s2))
            .RunWith(this.SinkProbe<ConsumeResult<Null, int>>(), Materializer);
    ...
    public async Task PlainPartitionedSource_should_not_leave_gaps_when_subsource_is_cancelled()
    {
        ...
        var consumedMessagesTask = KafkaConsumer
            .PlainPartitionedSource(CreateConsumerSettings<string>(group), Subscriptions.Topics(topic))
...
#pragma warning restore CS4014
            {
                ...
                source
                    .Select(message =>
                    {
                        ...
                        if (value % 10 == 0)
                        {
                            Log.Debug("[{0}] Reached message to fail: offset: [{1}], value: [{2}]",
                                message.TopicPartition, message.Offset, value);
    ...
    public void NativeKafkaIConsumerSeekShouldNotFail()
    {
        ...
        {
            new(new TopicPartition(topic, 0), 0),
            new(new TopicPartition(topic, 1), 0),
            new(new TopicPartition(topic, 2), 0)
        };
