// -----------------------------------------------------------------------
//  <copyright file="PlainSinkIntegrationTests.cs" company="Akka.NET Project">
...
public class PlainSinkIntegrationTests : KafkaIntegrationTests
{
    ...
    public async Task PlainSink_should_fail_stage_if_broker_unavailable()
    {
        ...
        var probe = Source
            ...
            .Select(record =>
                new Message<Null, string, NotUsed>(record, NotUsed.Instance) as IEnvelope<Null, string, NotUsed>)
