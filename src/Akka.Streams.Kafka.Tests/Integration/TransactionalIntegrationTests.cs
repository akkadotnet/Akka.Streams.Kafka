// -----------------------------------------------------------------------
//  <copyright file="TransactionalIntegrationTests.cs" company="Akka.NET Project">
...
public class TransactionalIntegrationTests : KafkaIntegrationTests
{
    ...
    [Fact(Skip =
        "Missing producer transactions support, see https://github.com/akkadotnet/Akka.Streams.Kafka/issues/85")]
        ...
        var control = KafkaConsumer.TransactionalSource(settings, Subscriptions.Topics(sourceTopic))
            ...
            .Select(message =>
            {
                return ProducerMessage.Single(
                    new ProducerRecord<Null, string>(targetTopic, message.Record.Message.Key,
    ...
    private DrainingControl<IImmutableList<ConsumeResult<Null, string>>> ConsumeStrings(string topic, int count) =>
        KafkaConsumer.PlainSource(CreateConsumerSettings<string>(CreateGroup(1)), Subscriptions.Topics(topic))
            .Take(count)
