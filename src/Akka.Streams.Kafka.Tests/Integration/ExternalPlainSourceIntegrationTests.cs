// -----------------------------------------------------------------------
//  <copyright file="ExternalPlainSourceIntegrationTests.cs" company="Akka.NET Project">
...
public class ExternalPlainSourceIntegrationTests : KafkaIntegrationTests
{
    ...
    public async Task ExternalPlainSource_with_external_consumer_Should_work()
    {
        ...
        var consumer =
            Sys.ActorOf(KafkaConsumerActorMetadata.GetProps(CreateConsumerSettings<string>(group), _defaultDecider));

        //Manually assign topic partition to it
        var (control1, probe1) =
            CreateExternalPlainSourceProbe<string>(consumer, Subscriptions.Assignment(new TopicPartition(topic, 0)));
        var (control2, probe2) =
            CreateExternalPlainSourceProbe<string>(consumer, Subscriptions.Assignment(new TopicPartition(topic, 1)));

        // Produce messages to partitions
        await ProduceStrings(new TopicPartition(topic, new Partition(0)), Enumerable.Range(1, elementsCount),
            ProducerSettings);
        await ProduceStrings(new TopicPartition(topic, new Partition(1)), Enumerable.Range(1, elementsCount),
            ProducerSettings);
        ...
        AwaitCondition(() =>
            control1.IsShutdown.IsCompletedSuccessfully && control2.IsShutdown.IsCompletedSuccessfully);
    ...
    public async Task ExternalPlainSource_should_be_stopped_on_serialization_error_only_when_requested_messages()
    {
        ...
        var (control1, probe1) =
            CreateExternalPlainSourceProbe<int>(consumer, Subscriptions.Assignment(new TopicPartition(topic, 0)));
        var (control2, probe2) =
            CreateExternalPlainSourceProbe<int>(consumer, Subscriptions.Assignment(new TopicPartition(topic, 1)));
        var (control3, probe3) =
            CreateExternalPlainSourceProbe<int>(consumer, Subscriptions.Assignment(new TopicPartition(topic, 2)));
        ...
        AwaitCondition(() =>
            control1.IsShutdown.IsCompleted && control2.IsShutdown.IsCompleted &&
            control3.IsShutdown.IsCompletedSuccessfully);
