// -----------------------------------------------------------------------
//  <copyright file="SourceWithOffsetContextStage.cs" company="Akka.NET Project">
...
internal class SourceWithOffsetContextStage<K, V> : KafkaSourceStage<K, V, (ConsumeResult<K, V>, ICommittableOffset)>
{
    ...
    {
        _metadataFromMessage = metadataFromMessage ?? (_ => string.Empty);
        Settings = settings;
    ...
    {
        var logic = new SingleSourceStageLogic<K, V, (ConsumeResult<K, V>, ICommittableOffset)>(shape, Settings,
            Subscription,
            inheritedAttributes,
            GetMessageBuilder);
    ...
    private OffsetContextBuilder<K, V> GetMessageBuilder(
        BaseSingleSourceLogic<K, V, (ConsumeResult<K, V>, ICommittableOffset)> logic)
    {
