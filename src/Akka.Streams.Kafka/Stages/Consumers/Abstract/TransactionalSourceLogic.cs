// -----------------------------------------------------------------------
//  <copyright file="TransactionalSourceLogic.cs" company="Akka.NET Project">
    ...
    ITransactionalMessageBuilderStage<K, V, TransactionalMessage<K, V>>
{
    ...
    public TransactionalSourceLogic(
        ...
        Func<ITransactionalMessageBuilderStage<K, V, TransactionalMessage<K, V>>, TransactionalMessageBuilder<K, V>>
            messageBuilderFactory)
        : base(shape, settings, subscription, attributes,
            logic => messageBuilderFactory((ITransactionalMessageBuilderStage<K, V, TransactionalMessage<K, V>>)logic))
    {
        _shape = shape;
        _settings = settings;
        ...
        _committedMarker =
            new Lazy<ICommittedMarker>(() => new CommittedMarkerRef(SourceActor.Ref, _settings.CommitTimeout));
    }
    ...
    private void DrainHandling((IActorRef, object) arg, Action<(IActorRef, object)> defaultHandler)
    {
        ...
        switch (msg)
        {
            case KafkaConsumerActorMetadata.Internal.Committed committed:
                _inFlightRecords.Committed(committed.Offsets.ToImmutableDictionary(offset => offset.TopicPartition,
                    offset => new Offset(offset.Offset - 1)));
            ...
            case Drain drain:
                ...
                else
                {
                    ...
                    Materializer.ScheduleOnce(_settings.DrainingCheckInterval,
                        () =>
                        {
                            SourceActor.Ref.Tell(new Drain(drain.Partitions,
                                drain.DrainedConfirmationRef.GetOrElse(sender).AsOption(),
                                drain.DrainedConfirmationMessage));
                        });
                }
    ...
    public TransactionalMessage<K, V> CreateMessage(ConsumeResult<K, V> record) =>
        _messageBuilder.CreateMessage(record);

    /// <inheritdoc />
    public string GroupId
    {
        get { return _settings.GroupId; }
    }

    /// <inheritdoc />
    public ICommittedMarker CommittedMarker
    {
        get { return _committedMarker.Value; }
    }

    /// <inheritdoc />
    public void OnMessage(ConsumeResult<K, V> message) => _inFlightRecords.Add(ImmutableDictionary
        .Create<TopicPartition, Offset>().Add(message.TopicPartition, message.Offset));

    /// <inheritdoc />
    protected override void StopConsumerActor() => SourceActor.Ref.Tell(
        new Drain(_inFlightRecords.Assigned, ConsumerActor.AsOption(),
            KafkaConsumerActorMetadata.Internal.Stop.Instance), SourceActor.Ref);
    ...
    private class TransactionalAssignmentHandler : IPartitionEventHandler
    {
        ...
        public void OnRevoke(IImmutableSet<TopicPartitionOffset> revokedTopicPartitions,
            IRestrictedConsumer consumer) => throw new NotImplementedException();
    ...
    {
        public Drain(IImmutableSet<TopicPartition> partitions, Option<IActorRef> drainedConfirmationRef,
            object drainedConfirmationMessage)
        {
    ...
    private sealed class CommittedMarkerRef : ICommittedMarker
    {
        ...
        {
            var topicPartitionOffsets = offsets.Select(o => new TopicPartitionOffset(o.Key, o.Value.Offset))
                .ToImmutableHashSet();
    ...
    {
        private IImmutableDictionary<TopicPartition, Offset> _inFlightRecords =
            ImmutableDictionary<TopicPartition, Offset>.Empty;
        ...
        /// </summary>
        public static InFlightRecords Empty
        {
            get { return new InFlightRecords(); }
        }
        ...
        /// </summary>
        public void Add(IImmutableDictionary<TopicPartition, Offset> offsets) =>
            _inFlightRecords = _inFlightRecords.AddRange(offsets);

        /// <summary>
        /// Removes committed records from collection
        /// </summary>
        public void Committed(IImmutableDictionary<TopicPartition, Offset> committed) => _inFlightRecords =
            _inFlightRecords.Where(pair => !committed.ContainsKey(pair.Key) || committed[pair.Key] != pair.Value)
                .ToImmutableDictionary();

        /// <summary>
        /// Removes revoked partitions from collection
        /// </summary>
        public void Revoke(IEnumerable<TopicPartition> topicPartitions) =>
            _inFlightRecords = _inFlightRecords.RemoveRange(topicPartitions);
        ...
        /// </summary>
        public bool IsEmpty(IImmutableSet<TopicPartition> partitions) =>
            !partitions.Any(tp => _inFlightRecords.ContainsKey(tp));

        /// <summary>
        /// Gets list of assigned topic partitions
        /// </summary>
        public IImmutableSet<TopicPartition> Assigned
        {
            get { return _inFlightRecords.Keys.ToImmutableHashSet(); }
        }
