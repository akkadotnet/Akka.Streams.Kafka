// -----------------------------------------------------------------------
//  <copyright file="TransactionalProducerStage.cs" company="Akka.NET Project">
...
/// </summary>
internal class TransactionalProducerStage<K, V, TPassThrough> :
    GraphStage<FlowShape<IEnvelope<K, V, TPassThrough>, Task<IResults<K, V, TPassThrough>>>>,
    IProducerStage<K, V, TPassThrough, IEnvelope<K, V, TPassThrough>, IResults<K, V, TPassThrough>>
{
    ...
    {
        get { return _settings.FlushTimeout; }
    }
    ...
    public Inlet<IEnvelope<K, V, TPassThrough>> In { get; } = new("kafka.transactional.producer.in");
    public Outlet<Task<IResults<K, V, TPassThrough>>> Out { get; } = new("kafka.transactional.producer.out");
    ...
    protected override GraphStageLogic CreateLogic(Attributes inheritedAttributes) =>
        new TransactionalProducerStageLogic<K, V, TPassThrough>(this, inheritedAttributes, _settings.EosCommitInterval);
}

internal class TransactionalProducerStageLogic<K, V, TPassThrough> : DefaultProducerStageLogic<K, V, TPassThrough,
    IEnvelope<K, V, TPassThrough>, IResults<K, V, TPassThrough>>
{
    ...
    {
        SetHandler(_stage.Out, () => TryPull(_stage.In));
    ...
    private void CommitTransaction(NonemptyTransactionBatch batch, bool beginNewTransaction)
    {
        ...
        batch.InternalCommit().ContinueWith(t => { _onInternalCommitCallback.Invoke(); });
    ...
    private void BeginTransaction() => Log.Debug("Beginning new transaction");

    // TODO: Add producer work with transactions
    // producer.beginTransaction()
    private void AbortTransaction() => Log.Debug("Aborting transaction");
    ...
    private class NonemptyTransactionBatch : ITransactionBatch
    {
        ...
        public NonemptyTransactionBatch(PartitionOffsetCommittedMarker head,
            IImmutableDictionary<GroupTopicPartition, Offset>? tail = null)
        {
            _head = head;
            ...
            Offsets = _tail.AddRange([
                new KeyValuePair<GroupTopicPartition, Offset>(head.GroupTopicPartition, highestOffset)
            ]);
        }
        ...
        public ITransactionBatch Updated(PartitionOffsetCommittedMarker partitionOffset)
        {
            if (partitionOffset.GroupId != GroupId)
                throw new ArgumentException(
                    $"Transaction batch must contain messages from exactly 1 consumer group. {partitionOffset.GroupId} != {GroupId}");
