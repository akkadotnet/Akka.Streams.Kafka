// -----------------------------------------------------------------------
//  <copyright file="DefaultProducerStage.cs" company="Akka.NET Project">
...
internal sealed class DefaultProducerStage<K, V, P, TIn, TOut> : GraphStage<FlowShape<TIn, Task<TOut>>>,
    IProducerStage<K, V, P, TIn, TOut>
    ...
    where TOut : IResults<K, V, P>
{
    ...
    {
        get { return Settings.FlushTimeout; }
    }
    ...
    {
        Settings = settings;
        ProducerProvider = errorHandler =>
            customProducerProvider?.Invoke() ?? Settings.CreateKafkaProducer(errorHandler);
    ...
    protected override GraphStageLogic CreateLogic(Attributes inheritedAttributes) =>
        new DefaultProducerStageLogic<K, V, P, TIn, TOut>(this, inheritedAttributes);
}
    ...
    where TOut : IResults<K, V, P>
{
    ...
    protected readonly AtomicCounter AwaitingConfirmation = new(0);

    public DefaultProducerStageLogic(IProducerStage<K, V, P, TIn, TOut> stage, Attributes attributes) :
        base(stage.Shape)
    {
        _stage = stage;

        // TODO: Move this to the GraphStage.InitialAttribute when it is fixed (https://github.com/akkadotnet/akka.net/issues/5388)
        var supervisionStrategy =
            attributes.GetAttribute(new ActorAttributes.SupervisionStrategy(new DefaultProducerDecider<K, V>().Decide));
                ...
                switch (msg)
                {
                    case Message<K, V, P> message:
                    {
                        ...
                        try
                        {
                            var callback = BuildSendCallback(
                                result,
                                report => { result.SetResult(new Result<K, V, P>(report, message)); },
                                OnProduceFailure);
                    ...
                    case MultiMessage<K, V, P> multiMessage:
                    {
                        var tasks = multiMessage.Records.Select(record =>
                        {
                            ...
                            var callback = BuildSendCallback(
                                result,
                                report => { result.SetResult(new MultiResultPart<K, V>(report, record)); },
                                OnProduceFailure);
                            try
                            {
                        ...
                        {
                            PostSend(msg);
                            var resultTask = Task.WhenAll(tasks!).ContinueWith(t =>
                                new MultiResult<K, V, P>(t.Result.ToImmutableHashSet(), multiMessage.PassThrough) as
                                    IResults<K, V, P>);
                            // compiler ceremony: the Task result is of type IResults<K, V, P> and we know it is a Result<K, V, P>
                            // but we can't cast it to IResults<K, V, P> because it is not covariant
                            // TODO: probably should redesign the generic parameters to avoid this entirely
                    ...
                    case PassThroughMessage<K, V, P> passThroughMessage:
                    {
                        PostSend(msg);
                        var resultTask =
                            Task.FromResult<IResults<K, V, P>>(
                                new PassThroughResult<K, V, P>(passThroughMessage.PassThrough));
                        // compiler ceremony: the Task result is of type IResults<K, V, P> and we know it is a Result<K, V, P>
                        // but we can't cast it to IResults<K, V, P> because it is not covariant
                        // TODO: probably should redesign the generic parameters to avoid this entirely
                        Push(stage.Out!, resultTask as Task<TOut>);
