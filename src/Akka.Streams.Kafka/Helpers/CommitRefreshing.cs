// -----------------------------------------------------------------------
//  <copyright file="CommitRefreshing.cs" company="Akka.NET Project">
...
internal interface ICommitRefreshing<K, V>
{
    ...
    void AssignedPositions(IImmutableSet<TopicPartition> assignedPartitions,
        IImmutableSet<TopicPartitionOffset> assignedOffsets);

    void AssignedPositions(IImmutableSet<TopicPartition> assignedTopicPartitions, IConsumer<K, V> consumer,
...
internal static class CommitRefreshing
{
    ...
    public class NoOp<K, V> : ICommitRefreshing<K, V>
    {
        ...
        {
            get { return ImmutableHashSet<TopicPartitionOffset>.Empty; }
        }
        ...
        public void AssignedPositions(IImmutableSet<TopicPartition> assignedPartitions,
            IImmutableSet<TopicPartitionOffset> assignedOffsets)
        {
        }

        public void AssignedPositions(IImmutableSet<TopicPartition> assignedTopicPartitions, IConsumer<K, V> consumer,
            TimeSpan positionTimeout)
        {
        }
    }
    ...
    public class Impl<K, V> : ICommitRefreshing<K, V>
    {
        ...
        private IImmutableDictionary<TopicPartition, Offset> _requestedOffsets =
            ImmutableDictionary<TopicPartition, Offset>.Empty;

        private IImmutableDictionary<TopicPartition, Offset> _committedOffsets =
            ImmutableDictionary<TopicPartition, Offset>.Empty;

        private IImmutableDictionary<TopicPartition, DateTime> _refreshDeadlines =
            ImmutableDictionary<TopicPartition, DateTime>.Empty;
        ...
        public IImmutableSet<TopicPartitionOffset> RefreshOffsets
        {
            get
            {
                ...
                {
                    return _committedOffsets.Where(tpo =>
                            overdueTopicPartitions.Contains(tpo.Key) && _requestedOffsets.Contains(tpo))
        ...
        public void UpdateRefreshDeadlines(IImmutableSet<TopicPartition> topicPartitions) => _refreshDeadlines =
            _refreshDeadlines.SetItems(
                topicPartitions.ToImmutableDictionary(p => p, p => DateTime.UtcNow.Add(_commitRefreshInterval)));
