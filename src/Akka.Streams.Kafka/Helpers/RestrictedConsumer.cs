// -----------------------------------------------------------------------
//  <copyright file="RestrictedConsumer.cs" company="Akka.NET Project">
...
internal class RestrictedConsumer<K, V> : IRestrictedConsumer
{
    ...
    {
        get { return _consumer.Assignment; }
    }

    /// <inheritdoc />
    public List<TopicPartitionOffset> BeginningOffsets(IEnumerable<TopicPartition> topicPartitions)
    {
        var timestamps = topicPartitions
            .Select(tp => new TopicPartitionTimestamp(tp, new Timestamp(0, TimestampType.NotAvailable))).ToList();
    ...
    /// <inheritdoc />
    public void Committed(IEnumerable<TopicPartition> topicPartitions) =>
        _consumer.Committed(topicPartitions, _duration);
