// -----------------------------------------------------------------------
//  <copyright file="KafkaProducer.cs" company="Akka.NET Project">
...
public static class KafkaProducer
{
    ...
    public static Sink<ProducerRecord<TKey, TValue>, Task<Done>> PlainSink<TKey, TValue>(
        ...
        Flow
            ...
            .Select(record =>
                new Message<TKey, TValue, NotUsed>(record, NotUsed.Instance) as IEnvelope<TKey, TValue, NotUsed>)
    ...
    /// </para>
    /// </summary>
    public static Flow<IEnvelope<TKey, TValue, TPassThrough>, IResults<TKey, TValue, TPassThrough>, NotUsed> FlexiFlow<
        TKey, TValue, TPassThrough>(
        ProducerSettings<TKey, TValue> settings)
    {
        var flow = Flow.FromGraph(
                new DefaultProducerStage<TKey, TValue, TPassThrough, IEnvelope<TKey, TValue, TPassThrough>,
                    IResults<TKey, TValue, TPassThrough>>(
                    settings,
                    true))
    ...
    [ApiMayChange]
    public static FlowWithContext<IEnvelope<K, V, NotUsed>, C, IResults<K, V, C>, C, NotUsed> FlowWithContext<K, V, C>(
    ...
    [ApiMayChange]
    public static FlowWithContext<IEnvelope<K, V, NotUsed>, C, IResults<K, V, C>, C, NotUsed> FlowWithContext<K, V, C>(
        ProducerSettings<K, V> settings, IProducer<K, V> producer) =>
    ...
    /// </summary>
    [InternalApi]
    public static Flow<IEnvelope<K, V, GroupTopicPartitionOffset>, IResults<K, V, GroupTopicPartitionOffset>, NotUsed>
        TransactionalFlow<K, V>(
            ...
            string transactionalId)
    {
        ...
        var flow = Flow
            .FromGraph(new TransactionalProducerStage<K, V, GroupTopicPartitionOffset>(true, transactionalSettings))
    ...
    public static Sink<IEnvelope<K, V, GroupTopicPartitionOffset>, Task<Done>> TransactionalSink<K, V>(
        ...
        TransactionalFlow(settings, transactionalId)
            .ToMaterialized(Sink.Ignore<IResults<K, V, GroupTopicPartitionOffset>>(), Keep.Right);

    private static Flow<IEnvelope<K, V, TPassThrough>, IResults<K, V, TPassThrough>, NotUsed> FlowWithDispatcher<K, V,
        TPassThrough>(
        ProducerSettings<K, V> settings,
        Flow<IEnvelope<K, V, TPassThrough>, IResults<K, V, TPassThrough>, NotUsed> flow) =>
        string.IsNullOrEmpty(settings.DispatcherId)
            ? flow
