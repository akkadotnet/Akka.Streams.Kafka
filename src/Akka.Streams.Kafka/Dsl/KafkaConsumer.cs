// -----------------------------------------------------------------------
//  <copyright file="KafkaConsumer.cs" company="Akka.NET Project">
...
public static class KafkaConsumer
{
    ...
    /// </summary>
    public static Source<ConsumeResult<K, V>, IControl> PlainSource<K, V>(ConsumerSettings<K, V> settings,
        ISubscription subscription) => Source.FromGraph(new PlainSourceStage<K, V>(settings, subscription));
    ...
    public static Source<ConsumeResult<K, V>, IControl> PlainExternalSource<K, V>(
        ...
        Source.FromGraph(new ExternalPlainSourceStage<K, V>(consumer, subscription, autoCreateTopics));
    ...
    /// </summary>
    public static Source<CommittableMessage<K, V>, IControl> CommittableSource<K, V>(ConsumerSettings<K, V> settings,
        ISubscription subscription) => Source.FromGraph(new CommittableSourceStage<K, V>(settings, subscription));
    ...
    /// </summary>
    public static Source<(TopicPartition, Source<ConsumeResult<K, V>, NotUsed>), IControl> PlainPartitionedSource<K, V>(
        ConsumerSettings<K, V> settings,
        IAutoSubscription subscription) =>
        Source.FromGraph(new PlainSubSourceStage<K, V>(settings, subscription,
    ...
    /// </summary>
    public static Source<CommittableMessage<K, V>, IControl> CommitWithMetadataSource<K, V>(
        ConsumerSettings<K, V> settings, ISubscription subscription,
        Func<ConsumeResult<K, V>, string> metadataFromRecord) =>
        Source.FromGraph(new CommittableSourceStage<K, V>(settings, subscription, metadataFromRecord));
    ...
    public static SourceWithContext<ConsumeResult<K, V>, ICommittableOffset, IControl> SourceWithOffsetContext<K, V>(
        ...
        Source.FromGraph(new SourceWithOffsetContextStage<K, V>(settings, subscription, metadataFromRecord))
    ...
    public static Source<CommittableMessage<K, V>, IControl> CommittableExternalSource<K, V>(
        ...
        bool autoCreateTopics = false) =>
        Source.FromGraph(new ExternalCommittableSourceStage<K, V>(consumer, groupId, commitTimeout, subscription,
            autoCreateTopics));

    /// <summary>
    /// The same as <see cref="PlainPartitionedSource{K,V}"/> but with offset commit support.
    /// </summary>
    public static Source<(TopicPartition, Source<CommittableMessage<K, V>, NotUsed>), IControl>
        CommittablePartitionedSource<K, V>(
            ConsumerSettings<K, V> settings, IAutoSubscription subscription) =>
        Source.FromGraph(new CommittableSubSourceStage<K, V>(settings, subscription));
    ...
    /// </summary>
    public static Source<ConsumeResult<K, V>, IControl> AtMostOnceSource<K, V>(ConsumerSettings<K, V> settings,
        ISubscription subscription)
    {
        return CommittableSource(settings, subscription).SelectAsync(1, async message =>
        {
#pragma warning disable CS0618 // Type or member is obsolete
            await message.CommitableOffset
                .Commit(); // TODO: need access to an internal interface or base class method here
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    /// </summary>
    public static Source<(TopicPartition, Source<CommittableMessage<K, V>, NotUsed>), IControl>
        CommitWithMetadataPartitionedSource<K, V>(
            ConsumerSettings<K, V> settings, IAutoSubscription subscription,
            Func<ConsumeResult<K, V>, string> metadataFromRecord) =>
        Source.FromGraph(new CommittableSubSourceStage<K, V>(settings, subscription, metadataFromRecord));
    ...
    /// </summary>
    public static Source<(TopicPartition, Source<ConsumeResult<K, V>, NotUsed>), IControl>
        PlainPartitionedManualOffsetSource<K, V>(
            ConsumerSettings<K, V> settings,
            IAutoSubscription subscription,
        ...
        Source.FromGraph(new PlainSubSourceStage<K, V>(
            settings,
            subscription,
            Option<Func<IImmutableSet<TopicPartition>, Task<IImmutableSet<TopicPartitionOffset>>>>.Create(
                getOffsetsOnAssign),
            onRevoke));
    ...
    /// </summary>
    public static Source<TransactionalMessage<K, V>, IControl>
        TransactionalSource<K, V>(ConsumerSettings<K, V> settings, ISubscription subscription) =>
        Source.FromGraph(new TransactionalSourceStage<K, V>(settings, subscription));
}